...
describe("Testing user model", () => {
    ...
    it("Password must be at least 8 characters long", async () => {
        const userData = { name: "Johnny", email: "johnny231@example.com", password: "12345673J" };
        try {
    ...
    it("The function 'readOne' must return one user", async () => {
        ...
        expect(oneUser).to.have.property("name", "Gabriela"); 
    });

    it("The function 'update' must update the user's data", async () => {
        const uid = "65e39235151655657c8857a9"; 
        const newData = { name: "Zaira" };

        const updatedUser = await users.update(uid, newData);

        expect(updatedUser).to.be.an("object");
        expect(updatedUser).to.have.property("name", "Mayra");
    });
});
